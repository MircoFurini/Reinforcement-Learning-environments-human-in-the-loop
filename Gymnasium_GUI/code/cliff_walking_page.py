...
import numpy as np
import importlib
from model.cliff_walking import CliffWalking
from datetime import datetime
import time
from copy import deepcopy

class CliffWalkingPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label_comunication = Label(self,
                           ...
                           fg="#8398E3",
                           bg="#FFFFFF")
        self.label_comunication.place(x=43, y=450)
        ...
        self.ax3.set_yticklabels(['Up','Right','Down','Left'])
    ...
    def button_start_clicked(self):
        ...
        self.cliffWalking = CliffWalking()
    ...
    def button_new_episode_clicked(self):
        ...
        self.cliffWalking_predict = deepcopy(self.cliffWalking)
        ...
        image_load = self.cliffWalking.render_image()
    ...
    def button_episode_suggestions_clicked(self):
        ...
        self.button_new_episode_clicked() #instructions are the same
        ...
        #first prediction
        observation = int(self.cliffWalking.get_state())
        self.action_predict = self.cliffWalking.learnedAction(observation)
    ...
    def button_qlearning_clicked(self): 
        ...
        while not self.terminated and not self.truncated:
            ...
            observation = int(self.cliffWalking.get_state())
            action = self.cliffWalking.learnedAction(observation)
            print(action)
            reward, steps, self.terminated, self.truncated, observation = self.cliffWalking.select_action(action)
        ...
        steps = self.cliffWalking.get_steps()
    ...
    def button_save_clicked(self): #save data and restart episode
        ...
        if self.episodes_arr == []:
            ...
            df.to_csv(f'report/cliff_walking_{current_datetime}.csv', encoding='utf-8', index = False)
            self.label_comunication.config(text=f"cliff_walking_{current_datetime}.csv saved!Please restart the env!", fg = "red")
    ...
    def button_score_clicked(self):
        ...
        for _ in range(0,10):
            ...
            while not terminated and not truncated:
                observation = int(self.cliffWalking.get_state())
                action = self.cliffWalking.learnedAction(observation)
                reward, steps, terminated, truncated, observation = self.cliffWalking.select_action(action)
    ...
    def update_graph(self, reward, steps, action, observation):   
        ...
        if self.check_buttons_suggestions_clicked:
            ...
            self.x_data_rew_predict = self.x_data_rew_predict[-1:]  
            self.y_data_rew_predict.append(self.reward_predict)
            self.y_data_rew_predict = self.y_data_rew_predict[-1:]  
            self.line_rew_predict.set_ydata(self.y_data_rew_predict)
            self.line_rew_predict.set_xdata(self.x_data_rew_predict)
    ...
    def button_up_clicked(self):
        ...
        if not self.terminated and not self.truncated:
            if self.check_buttons_suggestions_clicked:
                cliffWalking_predict_2 = deepcopy(self.cliffWalking_predict)
                self.reward_predict, steps_predict, terminated_predict, truncated_predict, observation_predict = cliffWalking_predict_2.select_action(self.action_predict)

                self.tot_reward_predict = self.tot_reward + self.reward_predict 

                self.cliffWalking_predict.select_action(0)

            reward, steps, self.terminated, self.truncated, observation = self.cliffWalking.select_action(0)
            image_load = self.cliffWalking.render_image()
            self.image_render = ImageTk.PhotoImage(image_load, master=self)
            self.label_1.config(image=self.image_render)
    ...
    def button_down_clicked(self):
        if not self.terminated and not self.truncated:
            if self.check_buttons_suggestions_clicked:
                ...
                self.cliffWalking_predict.select_action(2)

            reward, steps, self.terminated, self.truncated, observation = self.cliffWalking.select_action(2)
            image_load = self.cliffWalking.render_image()
            self.image_render = ImageTk.PhotoImage(image_load, master=self)
            self.label_1.config(image=self.image_render)
    ...
    def button_right_clicked(self):
        ...
        if not self.terminated and not self.truncated:
            if self.check_buttons_suggestions_clicked:
                ...
                self.cliffWalking_predict.select_action(1)

            reward, steps, self.terminated, self.truncated, observation = self.cliffWalking.select_action(1)
            image_load = self.cliffWalking.render_image()
            self.image_render = ImageTk.PhotoImage(image_load, master=self)
            self.label_1.config(image=self.image_render)
    ...
    def button_left_clicked(self):
        ...
        if not self.terminated and not self.truncated:
            if self.check_buttons_suggestions_clicked:
                ...
                self.cliffWalking_predict.select_action(3)

            reward, steps, self.terminated, self.truncated, observation = self.cliffWalking.select_action(3)
            image_load = self.cliffWalking.render_image()
            self.image_render = ImageTk.PhotoImage(image_load, master=self)
            self.label_1.config(image=self.image_render)
